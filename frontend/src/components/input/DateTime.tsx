...
const DateTime = ({ label, ...props }: BaseProps<string>): JSX.Element => {
  const [{ value }, { error }, { setValue }] = useField(props);
  const { className, required, disabled } = props;
  const id = generateId('datetime', label);
    ...
    <div className={className}>
      ...
      <div className="mt-1 relative rounded-md shadow-sm">
        <Flatpickr
          ...
          value={value}
          onChange={(date, currentDateString) => setValue(currentDateString, true)}
